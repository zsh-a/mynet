...
class TcpServer {
 ...
 public:
  ...
  TcpServer(int fd,std::string_view name, Callback cb) : fd_(fd),channel_(fd),name_(name), cb_(std::forward<Callback>(cb)) {}
  TcpServer(TcpServer&& other)
      : fd_(std::exchange(other.fd_, -1)),channel_(other.channel_),name_(std::move(other.name_)) ,cb_(other.cb_) {}
  Task<Connection> serve() {
    ...
    while (1) {
      ...
      auto ip_port = fmt::format("{}.{}.{}.{}:{}", addr->sin_addr.s_addr & 0xff,
  ...
  Task<Connection::Ptr> accept(){
    ...
    int client_fd =
        ::accept(fd_, reinterpret_cast<sockaddr*>(&remote_addr), &sock_len);
    if (client_fd == -1) co_return nullptr;
    auto addr = reinterpret_cast<sockaddr_in*>(&remote_addr);
    auto ip_port = fmt::format("{}.{}.{}.{}:{}", addr->sin_addr.s_addr & 0xff,
                          addr->sin_addr.s_addr >> 8 & 0xff,
                          addr->sin_addr.s_addr >> 16 & 0xff,
